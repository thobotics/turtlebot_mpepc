...
ControlLaw::ControlLaw(ControlLawSettings *c) { settings_ = c; }
...
double ControlLaw::get_ego_distance(geometry_msgs::Pose current_pose,
...
EgoPolar ControlLaw::convert_to_egopolar(
    geometry_msgs::Pose current_pose, geometry_msgs::Pose current_goal_pose) {
  EgoPolar coords;
...
geometry_msgs::Pose ControlLaw::convert_from_egopolar(
    geometry_msgs::Pose current_pose, EgoPolar current_goal_coords) {
  geometry_msgs::Pose current_goal_pose;

  double current_yaw = tf::getYaw(current_pose.orientation);

  current_goal_pose.position.x =
      current_pose.position.x +
      current_goal_coords.r * cos(current_yaw - current_goal_coords.delta);
  current_goal_pose.position.y =
      current_pose.position.y +
      current_goal_coords.r * sin(current_yaw - current_goal_coords.delta);
  current_goal_pose.position.z = 0;

  current_goal_pose.orientation = tf::createQuaternionMsgFromYaw(
      current_yaw - current_goal_coords.delta + current_goal_coords.theta);
...
double ControlLaw::get_kappa(EgoPolar current_ego_goal, double k1, double k2) {
  double kappa = 0;
  kappa =
      (-1 / current_ego_goal.r) *
      (k2 * (current_ego_goal.delta - atan(-1 * k1 * current_ego_goal.theta)) +
       (1 + k1 / (1 + pow(k1 * current_ego_goal.theta, 2))) *
           sin(current_ego_goal.delta));
  return kappa;
}

double ControlLaw::get_linear_vel(double kappa, EgoPolar current_ego_goal,
                                  double vMax) {
  double lin_vel = 0;
  lin_vel =
      min(settings_->m_V_MAX / settings_->m_R_THRESH * current_ego_goal.r,
          settings_->m_V_MAX /
              (1 + settings_->m_BETA * pow(abs(kappa), settings_->m_LAMBDA)));
...
geometry_msgs::Twist ControlLaw::get_velocity_command(
    geometry_msgs::Pose current_position, geometry_msgs::Pose goal, double k1,
    double k2, double vMax) {
  EgoPolar goal_coords = convert_to_egopolar(current_position, goal);

  return get_velocity_command(goal_coords, k1, k2, vMax);
}

geometry_msgs::Twist ControlLaw::get_velocity_command(EgoPolar goal_coords,
                                                      double vMax) {
  return get_velocity_command(goal_coords, settings_->m_K1, settings_->m_K2,
                              vMax);
}
